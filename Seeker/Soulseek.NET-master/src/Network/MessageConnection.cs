...
//     Modified: Replaced reflection-based connection fault wrapping for AOT.
//
    ...
    internal sealed class MessageConnection : Connection, IMessageConnection
    {
            ...
            : base(ipEndPoint, options, tcpClient)
        {
            ...
            Connected += (sender, e) =>
            {
                ...
                {
                    Task.Run(() => ReadContinuouslyAsync()).ForgetButThrowWhenFaulted(
                        exception => new ConnectionException(exception.Message, exception));
                }
                else
                {
