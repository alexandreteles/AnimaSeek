...
- **Capsules** — search chips and the selection-bar buttons (`CornerStyle = Capsule`), the room composer field (`UIKitFactory.ApplyCapsuleCorners`, which tracks the measured height at every Dynamic Type size), and the 44 pt circular send/jump-to-latest controls. Capsules mark compact interactive tokens only, never ordinary text containers.
...
- The shipped secondary-action surface is the **anchored action sheet** (`UIAlertControllerStyle.ActionSheet`, always popover-anchored on regular widths): row activation on Transfers/Search/Browse, sort and grouping pickers, user actions, and link actions all flow through it, with the current choice named textually. Modal forms use `FormSheet`/`PageSheet`. This is the recorded system; `UIContextMenuInteraction` and `UISheetPresentationController` detents are absent, and adopting them (context menus as *additive* discoverability, detents for the download review) is optional refinement noted in debt — never a replacement that hides essential actions behind long-press discovery.
...
- Composers ride the keyboard layout guide, keep drafts and focus on failure, and use a native Send return key. Sending/failed/retry states stay visible without blocking reading.
- Incoming bursts preserve the reading position unless already near latest; a labeled Jump to Latest control appears while reading history.
...
- Visual verification of the conversation-detail screens after the shared corner/border rework (message and room bubbles, the room composer capsule, and its trait-re-resolving border): launch routes cannot reach them, so they were outside the 2026-08-17 sweep.
